...
/// Favoritos guardados en el teléfono (no requiere cuenta).
...
class _FavoritesScreenState extends State<FavoritesScreen> {
  ...
  void _load() {
    final ids = FavoritesManager.instance.ids.value.toList();
    if (ids.isEmpty) {
      setState(() {
        _items = [];
        _loading = false;
    ...
    PropertyService.byIds(ids).then((data) {
      if (!mounted) return;
      setState(() {
        _items = data;
        _loading = false;
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : ValueListenableBuilder<Set<String>>(
              ...
              builder: (_, ids, __) {
                final list = _items.where((p) => ids.contains(p.id)).toList();
                if (list.isEmpty) {
                  return const Center(
                    child: Padding(
