...
export function PropertyCard({ property }: { property: PropertyWithImages }) {
  ...
  const isPremium = property.plan === "premium";
  const isOrange = property.featured && !isPremium;
  ...
  return (
    ...
    >
      {/* Barra de destacado arriba de la ficha */}
      {isPremium && (
        <div className="flex items-center gap-1.5 bg-[#33333A] px-3.5 py-1.5 text-[#E8C66A]">
          <CrownIcon />
      ...
      {isOrange && (
        <div className="flex items-center gap-1.5 bg-[#F97316] px-3.5 py-1.5 text-white">
          <StarIcon light />
          <span className="text-[13px] font-extrabold tracking-wide">
            DESTACADO
          </span>
...
function StarIcon({ light }: { light?: boolean }) {
  return (
    <svg width={14} height={14} viewBox="0 0 24 24" fill={light ? "#fff" : "#d97706"}>
...
function CrownIcon() {
  return (
    <svg width={15} height={15} viewBox="0 0 24 24" fill="#E8C66A">
      <path d="M5 16l-2-9 5.5 4L12 5l3.5 6L21 7l-2 9H5zm0 2h14v2H5v-2z" />
    </svg>
  );
}
