...
// Detalle de un inmueble dentro del sitio de marca blanca de la inmobiliaria.
// Solo muestra inmuebles que pertenecen a esa inmobiliaria.

export async function generateMetadata({
  params,
}: {
  params: Promise<{ agencia: string; slug: string }>;
}): Promise<Metadata> {
  const { agencia, slug } = await params;
  const supabase = getServerSupabase();
  const [agency, p] = await Promise.all([
    getAgencySiteBySlug(supabase, agencia).catch(() => null),
    getPropertyBySlug(supabase, slug).catch(() => null),
  ]);
  if (!agency || !p || p.owner_id !== agency.id) {
    return { title: "Inmueble no encontrado" };
  }

  const op = p.operation === "arriendo" ? "en arriendo" : "en venta";
  ...
  const url = `${agencySiteUrl(agency, SITE_URL)}/inmueble/${propertySlug(p)}`;
...
export default async function AgencyPropertyPage({
  params,
}: {
  params: Promise<{ agencia: string; slug: string }>;
}) {
  const { agencia, slug } = await params;
  const supabase = getServerSupabase();
  const [agency, property, amenities] = await Promise.all([
    getAgencySiteBySlug(supabase, agencia).catch(() => null),
    getPropertyBySlug(supabase, slug).catch(() => null),
  ...
  if (!agency) notFound();
  // Marca blanca: solo los inmuebles de ESTA inmobiliaria existen en su sitio.
  if (!property || property.owner_id !== agency.id) notFound();
  ...
  const url = `${agencySiteUrl(agency, SITE_URL)}/inmueble/${propertySlug(property)}`;
